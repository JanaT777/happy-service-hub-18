...
export const OtherRequestForm = ({ onBack, onSubmit, createdBy }: Props) => {
  ...
  const handleOrderSelect = (orderNum: string, order: MockOrder) => {
    ...
    setErrors(prev => {
      const { orderNumber: _, customerEmail: __, ...rest } = prev;
      return rest;
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isCRM && !selectedOrderNumber) {
      setErrors({ orderNumber: 'Vyberte objednávku.' });
      return;
    }

    const dataToValidate = isCRM
      ? { ...form, customerEmail: form.customerEmail || 'crm@internal.sk', orderNumber: form.orderNumber || selectedOrderNumber }
      : form;

    const result = otherSchema.safeParse(dataToValidate);
    if (!result.success) {
